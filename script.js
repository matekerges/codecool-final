...
const bookCard = (title, sub, text, number) => {
  return `
    <div class="card">
...
window.addEventListener('load', _ => {
  ...
  rootElement.insertAdjacentHTML('beforebegin', headerContent);

  const content = books.cards.map((book, index) => bookCard(book.title, book.sub, book.text, index + 1)).join('');
